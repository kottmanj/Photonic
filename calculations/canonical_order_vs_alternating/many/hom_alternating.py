...
class DoIt:
    def __init__(self, S: int, qpm: int, trotter_steps: int, order:int, samples: int, initial_state: str,
                 simulator: Simulator):
        self.S = S
        self.qpm = qpm
    ...
    def __call__(self, *args) -> PhotonicStateVector:
        ...
        setup.add_beamsplitter(path_a='a', path_b='b', t=0.25, steps=self.trotter_steps, order=self.order)
...
if __name__ == "__main__":
    ...
    print("parameters:\n", parameters)
    parameters['order'] = 0 
    counts1 = pool.map(func=DoIt(**parameters), iterable=range(0, runs//2))
    parameters['order'] = 1 
    counts2 = pool.map(func=DoIt(**parameters), iterable=range(0, runs//2))
    ...
    counts.plot(title="HOM-Counts for initial state " + parameters['initial_state'],
                label="S="+str(parameters['S'])+"\nqpm="+str(parameters['qpm'])+"\nsteps=" + str(parameters['trotter_steps']),
                filename="hom_S_"+str(parameters['S'])+"_qpm_"+str(parameters['qpm'])+"_steps_" + str(parameters['trotter_steps']) + "_alternating_1.pdf")
