...
if __name__ == "__main__":
    ...
    setup.add_beamsplitter(path_a='b', path_b='d', t=0.25, steps=trotter_steps)
    setup.add_doveprism(path='d', t=tq.Variable("x"))
    setup.add_beamsplitter(path_a='b', path_b='d', t=0.25, steps=trotter_steps)

    setup.add_beamsplitter(path_a='a', path_b='b', t=0.25, steps=trotter_steps)
    setup.add_doveprism(path='b', t=tq.Variable("y"))
    setup.add_beamsplitter(path_a='a', path_b='b', t=0.25, steps=trotter_steps)
    ...
    variables = {'t':1.0, 'angle0': 0.5, 'angle1': -1.0, 'x':0.0, 'y':0.0}
    ...
    values={'t':1.0, 'angle0':0.5 , 'angle1': -1.0, "x":numpy.random.uniform(0.0,6.0), "y":numpy.random.uniform(0.0,6.0)}

    for i in range(10):
        values = {'t': 1.0, 'angle0': 0.5, 'angle1': -1.0, "x": numpy.random.uniform(0.0, 6.0),
                  "y": numpy.random.uniform(0.0, 6.0)}
        print(tq.simulate(O, variables=values))

    result = tq.minimize(tol=1.e-3, initial_values=values, objective=O, method="bfgs", method_options={"eps":1.e-3}, gradient="2-poin", silent=False)
